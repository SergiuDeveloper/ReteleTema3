...
SuccessState Client::Connect(string serverIP, unsigned int serverPort)
{
    ...
    if (!operationSuccess)
        return SuccessState(false, ERROR_CONNECTION_FAILED(serverIP, serverPort));

    this->isConnected = true;

    return SuccessState(true, SUCCESS_CONNECTION_ESTABLISHED);
}
...
SuccessState Client::Disconnect()
{
    ...
    return SuccessState(true, SUCCESS_CONNECTION_CLOSED);
}
